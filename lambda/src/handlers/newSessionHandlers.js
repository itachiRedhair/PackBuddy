...
const initializePackingSession = require("./../utilities/packingListController")
  .setPackingSession;
const resetRemindMePackingList = require("./../utilities/packingListController")
  .resetRemindMePackingList;
const getIncompleteTripDetails = require("./../utilities/tripListController")
  .getIncompleteTripDetails;
...
const newSessionHandler = function() {
  getUserInfoFromFB.call(this).then(userInfo => {
    if (Object.keys(this.attributes).length === 0) {
      this.attributes[session.USER_INFO] = {};
      let greetingMessage =
        "Hi " + userInfo.first_name + ". " + messages.FIRST_TIME_MESSAGE;
      this.response.speak(greetingMessage).listen();
    ...
    getIncompleteTripDetails(userId)
      .then(incompleteTripDetails => {
        console.log(
          "in newsessionhandler incompletripdetails->",
          incompleteTripDetails
        );
        if (incompleteTripDetails !== null) {
          this.response
            .speak(
              messages.GREETING + " " + messages.START_QUESTION_WITH_REMINDER
            )
          ...
          this.response
            .speak(messages.GREETING + " " + messages.START_QUESTION)
...
const incompleteTripHandler = function() {
  ...
  getIncompleteTripDetails(userId).then(tripDetails => {
    if (
      ...
      this.response
        .speak(
          "It seems you don't have ongoing packing. You can instead try saying start new packing"
        )
        .listen("You can instead try saying start new packing");
      this.emit(":responseReady");
    }
...
const resumeOldPackingHandler = function() {
  this.attributes[session.IS_FIRST_ITEM] = true;
  let userId = this.event.session.user.userId;
  ...
  getIncompleteTripDetails(userId).then(tripDetails => {
    ...
    console.log(
      "in resume old packing, selected_category=>",
      tripDetails.selected_category
    );

    if (tripDetails.selected_category !== "null") {
      console.log("emiiting start selected ccategory intent");
      this.attributes[session.CURRENT_PACKING_CATEGORY_KEY] =
        tripDetails.selected_category;
