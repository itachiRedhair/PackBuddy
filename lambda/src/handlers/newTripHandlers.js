...
const generatePackingList = require("./../utilities/packingListController")
  .createPackingList;
...
const newSessionHandler = function() {
  this.response
    .speak(messages.NEW_TRIP_START_QUESTION)
    .listen(messages.NEW_TRIP_START_QUESTION_REPROMPT);
...
const helpHandler = function() {
  this.response.speak(messages.NEW_TRIP_HELP).listen(messages.NEW_TRIP_HELP);
  this.emit(":responseReady");
};
...
const unhandlerHandler = function() {
  this.response
    .speak(messages.NEW_TRIP_UNHANDLED)
...
const delegateSlotCollection = function() {
  ...
  if (this.event.request.dialogState === "STARTED") {
    ...
    const consentToken = this.event.context.System.user.permissions
      .consentToken;
...
const generateTrip = function() {
  return new Promise((resolve, reject) => {
    ...
    generatePackingList
      .call(this, toCity, date, duration)
      .then(list => {
        let count = 0;
...
let newTripHandlersWithState = Alexa.CreateStateHandler(
  states.NEW_TRIP,
  newTripHandlers
);
