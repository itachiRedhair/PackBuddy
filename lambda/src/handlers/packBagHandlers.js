...
const newSessionHandler = function() {
  this.response
    .speak(messages.PACK_BAG_START_QUESTION)
    .listen(messages.PACK_BAG_START_QUESTION_REPROMPT);
...
const packItemHandler = function() {
  ...
  switch (this.attributes[session.CURRENT_CATEGORY_PACKING_STATUS]) {
    ...
    case packingStatus.IN_PROGRESS:
      ...
      if (this.attributes[session.IS_FIRST_ITEM] === true) {
        this.attributes[session.PROMPT_QUEUE].push(
          messages.FIRST_PACK_ITEM_PROMPT
        );
        this.attributes[session.IS_FIRST_ITEM] = false;
      }
    ...
    case packingStatus.COMPLETED:
      ...
      this.attributes[session.CURRENT_PACKING_LIST][categoryKey][
        "all_packed"
      ] = true;
...
const helpHandler = function() {
  this.response.speak(messages.PACK_HELP).listen(messages.PACK_HELP);
  this.emit(":responseReady");
};
...
const yesHandler = function() {
  switch (this.attributes[session.CURRENT_TOTAL_PACKING_STATUS]) {
    case packingStatus.STARTED:
      ...
      break;
    case packingStatus.IN_PROGRESS:
      var currentPackingItemKey = this.attributes[
        session.CURRENT_PACKING_ITEM_KEY
      ];
      var currentPackingCategoryKey = this.attributes[
        session.CURRENT_PACKING_CATEGORY_KEY
      ];
      this.attributes[session.CURRENT_PACKING_LIST][currentPackingCategoryKey][
        "items"
      ][currentPackingItemKey]["status"] =
  ...
  switch (this.attributes[session.CURRENT_TOTAL_PACKING_STATUS]) {
    ...
    case packingStatus.IN_PROGRESS:
      ...
      this.attributes[session.CURRENT_PACKING_LIST][currentPackingCategoryKey][
        ...
        packingItemStatus.NOT_INTERESTED;
      this.emitWithState(intents.PackItemIntent);
      break;
    default:
      this.response.speak(
        "Maybe try something else. like saying help me packing"
      );
      this.emit(":responseReady");
  }
};
  ...
  switch (this.attributes[session.CURRENT_TOTAL_PACKING_STATUS]) {
    ...
    case packingStatus.IN_PROGRESS:
      ...
      this.attributes[session.CURRENT_PACKING_LIST][currentPackingCategoryKey][
        ...
        packingItemStatus.REMIND_LATER;
      this.attributes[session.CURRENT_PACKING_LIST][currentPackingCategoryKey][
        "remind_later"
      ] = true;
      this.emitWithState(intents.PackItemIntent);
...
const unhandledHandler = function() {
  this.response
    .speak(messages.PACK_ITEM_REPROMPT)
...
let packBagHandlersWithState = Alexa.CreateStateHandler(
  states.PACKING,
  packBagHandlers
);
