...
const getNotPackedCategories = require("./../utilities/packingListController")
  .getNotPackedCategories;
const getRemindMeStatus = require("./../utilities/packingListController")
  .getRemindMeStatus;
const resetRemindMePackingList = require("./../utilities/packingListController")
  .resetRemindMePackingList;
...
const listCategoryHandler = function() {
  ...
  let totalPackingStatus = this.attributes[
    session.CURRENT_TOTAL_PACKING_STATUS
  ];
  ...
  if (categories.length > 2) {
    ...
    if (totalPackingStatus === packingStatus.STARTED) {
      ...
      this.response
        .speak(message)
        .listen(messages.SELECT_CATEGORY_QUESTION_REPROMPT);
    ...
    let prompt =
      messages.REMAINING_SINGLE_CATEGORY_PROMPT + selectedCategory + ". ";
    ...
    if (totalPackingStatus === packingStatus.STARTED) {
      message +=
        "? Select one from " +
        packingList[categories[0]].name +
        " or " +
        packingList[categories[1]].name;
    ...
    if (getRemindMeStatus.call(this) === true) {
      ...
      this.response
        .speak(messages.REMIND_ITEMS_PACK_QUESTION)
        .listen(messages.REMIND_ITEMS_PACK_QUESTION);
      this.emit(":responseReady");
    } else {
...
const selectCategoryHandler = function() {
  let selectedCategory = this.event.request.intent.slots.selectedCategory
...
const startSelectedCategoryHandler = function() {
  this.attributes[session.CURRENT_CATEGORY_PACKING_STATUS] =
    packingStatus.IN_PROGRESS;
  this.attributes[session.CURRENT_TOTAL_PACKING_STATUS] =
    packingStatus.IN_PROGRESS;
  this.handler.state = states.PACKING;
...
const helpHandler = function() {
  this.response
    .speak(messages.SELECT_CATEGORY_HELP)
...
const unhandledHandler = function() {
  this.response
    .speak(messages.SELECT_CATEGORY_UNHANDLED)
...
categorySelectHandlers[
  intents.StartSelectedCategoryIntent
] = startSelectedCategoryHandler;

let categorySelectHandlersWithState = Alexa.CreateStateHandler(
