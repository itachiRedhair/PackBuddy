...
let getRemindMeStatus = require("./../utilities/packingListController")
  .getRemindMeStatus;
//import constants
...
function updatePackingList() {
  ...
  let selectedCategory = this.attributes[session.CURRENT_PACKING_CATEGORY_KEY]
  ...
  return new Promise((resolve, reject) => {
    ...
    var params = {
      ...
      UpdateExpression:
        "SET trip_list.#ti.#pl = :pl, trip_list.#ti.#rm = :rms, trip_list.#ti.#ps = :ps, trip_list.#ti.#sc = :sc ",
      ExpressionAttributeValues: {
        ":pl": packingList,
        ":rms": remindMeStatus,
        ":ps": packingStatus,
    ...
    documentClient.update(params, (err, data) => {
      if (err) {
        console.log(
          "Error while updating dynamodb in updatePackingList(), err=>",
          err
        );
        reject(err);
      } else {
...
function initializeDynamoDB(userId) {
  return new Promise((resolve, reject) => {
    ...
    getAllItemsData(userId)
      .then(data => {
        console.log(
          "in checkIf table data exist call, data=>",
          data,
          JSON.stringify(data),
...
function getAllItemsData(userId) {
  return new Promise((resolve, reject) => {
    ...
    var params = {
      ...
      AttributesToGet: ["trip_list", "last_trip_key"]
    };
