...
const fetchWeatherCustomizedList = require("./customizedList")
  .getWeatherCustomizedList;
const getGenderCustomizedList = require("./customizedList")
  .getGenderCustomizedList;
...
function createPackingList(destination, date, duration) {
  ...
  return new Promise((resolve, reject) => {
    getCustomizedLists(userInfo, destination, date, duration)
      .then(customizedListArray => {
        ...
        customizedListArray.forEach(customizedList => {
          ...
          keys.forEach(key =>
            Object.assign(fixedList[key]["items"], customizedList[key]["items"])
          );
        });
...
function getCustomizedLists(userInfo, destination, date, duration) {
  return new Promise((resolve, reject) => {
    ...
    Promise.all(promiseArr).then(customizedListArray => {
  ...
  for (var categoryKey in packingList) {
    for (var itemKey in packingList[categoryKey].items) {
      ...
      if (item.status === packingItemStatus.REMIND_LATER) {
        this.attributes[session.CURRENT_PACKING_LIST][categoryKey]["items"][
          itemKey
        ]["status"] =
          packingItemStatus.NOT_PACKED;
